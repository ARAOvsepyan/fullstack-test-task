...
class ThesisController {
	...
	async getAll(req, res, next) {
		try {
			...
			console.log(professor.count);
		
			if (professor.count <= 0) {
				return next(ApiError.NotFound("Преподователь не найден"));
			}

			const map = new Map(professor.rows.map(({title, professorId ,professor, student}) => [professorId, {
				id: professorId,
				full_name: [professor.last_name,  professor.first_name, professor.patronymic].join(' '), 
				callRecords: [] 
			}])); 
			for (let {professorId, student, title} of professor.rows) {
	...
	async create(req, res, next) {
		try {
			...
			console.log(thesis_info);

			const canditate = await StudentModel.findOne({where: {
				[Op.and]: [
					{
